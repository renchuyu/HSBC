...
public class PriceLimiter {
    ...
    private final Hashtable<String, AbstractMap.SimpleEntry> limits = new Hashtable<String, AbstractMap.SimpleEntry>();

    //priceReferenceTable for different instruments with instrument as key,
    private final Hashtable<String, PriceReference> priceReferenceTable = new Hashtable<String, PriceReference>();
        ...
        if (scenario != null) {
            switch (scenario.toUpperCase()) {
                ...
                case Constants.SCENARIOS_DISADVANTAGE:
                    ...
                    System.out.println(" new validation scenarios:" + scenario);
                    break;
    ...
    private Message processCommand(String instrument, String side, BigDecimal price) throws Exception {
        ...
        switch (limit.getKey().toString().toUpperCase()) {
            ...
            case Constants.VARIATION_TYPE_PERCENTAGE:
                //order price - reference price is over the percentage
                BigDecimal variationPercentage = (price.subtract(priceReference.getReferencePrice()).divide(priceReference.getReferencePrice(),3, RoundingMode.HALF_UP));
                ...
                if (variationPercentage.floatValue() >= ((BigDecimal)limit.getValue()).floatValue()) {
                    Message message = new Message(true, variationStrPecentage,pecentageStr + ">=" + formatToPercentage((float) limit.getValue())+ ", block") ;
                    //handle the scenario before return
                    return scenarioHandling(side,price,priceReference,message);
                }
                else {
                    Message message = new Message(false,  variationStrPecentage,  pecentageStr + "<" + formatToPercentage((float) limit.getValue())+ ", pass") ;
                    //handle the scenario before return
                    return scenarioHandling(side,price,priceReference,message);
                }
